...
- 期约的状态表示期约是否完成 
  ...
  - 兑现 *fulfilled* —— *pending → fulfilled*，内部生成一个私有的内部值 *value*
  - 拒绝 *rejected* —— *pending → rejected*，内部生成一个私有的内部理由 *reason*

- 状态切换不可逆，尝试再次切换状态会静默失败

- 状态是**私有**的，不能直接通过 *JavaScript* 检测到 —— 避免根据读取到的期约状态以同步方式处理期约对象
- 状态不允许被外部 *JavaScript* 代码修改约故意将异步行为封装起来，从而隔离外部代码
...
- 返回一个新的期约实例  —— 与状态无关，原样后传
  ...
  setTimeout(console.log,0,p2);//Promise {<resolve>: "foo"}
  setTimeout(console.log,0,p3);//Promise {<resolve>: "foo"}
  setTimeout(console.log,0,p4);//Promise {<resolve>: "foo"}
  setTimeout(console.log,0,p5);//Promise {<resolve>: "foo"}
  setTimeout(console.log,0,p6);//Promise {<resolve>: "foo"}
  setTimeout(console.log,0,p7);//Promise {<resolve>: "foo"}
  setTimeout(console.log,0,p8);//Promise {<resolve>: "foo"}
  ```
